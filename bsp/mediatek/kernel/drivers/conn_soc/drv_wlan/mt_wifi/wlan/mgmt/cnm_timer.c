    ...
    )
{
	...
	printk("[wlan] del prTimerList = 0x%p 0x%p %d %d\n", 
	&prRootTimer->rLinkHead, prTimer, 
	fgAcquireSpinlock, prRootTimer->rLinkHead.u4NumElem); 
	printk("[wlan] del timer->prev = 0x%p, timer->next = 0x%p\n", 
	prTimer->rLinkEntry.prPrev, prTimer->rLinkEntry.prNext);
    ...
    )
{
	...
	printk("[wlan] add prTimerList = 0x%p 0x%p\n", prTimerList, prTimer); 
	printk("[wlan] add timer->prev = 0x%p, timer->next = 0x%p\n",prTimer->rLinkEntry.prPrev, prTimer->rLinkEntry.prNext);
    }
