...
extern LCM_PARAMS *lcm_params;//jixu add
unsigned int EnableLenovoBL = 1;//jixu add
...
static void mt65xx_led_set(struct led_classdev *led_cdev, enum led_brightness level)
{
	...
	#ifdef LENOVO_BACKLIGHT_LIMIT//jixu add
	if((level>0)&&(lcm_params!=NULL)&&(strcmp(led_data->cust.name,"lcd-backlight") == 0)) {
		...
		bl_bsp_def = (lcm_params->bl_bsp.def==0)?90:(lcm_params->bl_bsp.def);
		...
		//printk("[JX] %s EnableLenovoBL=%d\n",__func__,EnableLenovoBL);
		//we must confirm the bl_app value was set correct in lcm drivers
		if((EnableLenovoBL)&&(lcm_params->bl_app.def==APP_BL_DEFAULT)){
