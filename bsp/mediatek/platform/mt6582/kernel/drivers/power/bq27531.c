...
unsigned char temph;
    ...
    for (i=2;i<bq27531_REG_NUM;i++)
    {
        ...
        printk("[0x%x]=0x%x, l=0x%x, h=0x%x ", i, bq27531_reg[i], templ, temph);        
    }
    printk("\n");
}
...
/**********************************************************
  ...
  *
  *********************************************************/
kal_uint8 g_reg_value_bq27531=0;
static ssize_t show_bq27531_access(struct device *dev,struct device_attribute *attr, char *buf)
{
    battery_xlog_printk(BAT_LOG_FULL,"[show_fan5405_access] 0x%x\n", g_reg_value_bq27531);
    return sprintf(buf, "%u\n", g_reg_value_bq27531);
}
static ssize_t store_bq27531_access(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    int ret=0;
    char *pvalue = NULL;
    ...
    battery_xlog_printk(BAT_LOG_FULL,"[store_bq27531_access] \n");
    
    if(buf != NULL && size != 0)
    {
        battery_xlog_printk(BAT_LOG_FULL,"[store_bq27531_access] buf is %s and size is %d \n",buf,size);
        reg_address = simple_strtoul(buf,&pvalue,16);
        
        if(size > 3)
        {        
            reg_value = simple_strtoul((pvalue+1),NULL,16);        
            battery_xlog_printk(BAT_LOG_FULL,"[store_bq27531_access] write bq27531 reg 0x%x with value 0x%x !\n",reg_address,reg_value);
            ret=bq27531_config_interface(reg_address, reg_value, 0xFF, 0x0);
        }
        else
        {    
            ret=bq27531_read_interface(reg_address, &g_reg_value_bq27531, 0xFF, 0x0);
            battery_xlog_printk(BAT_LOG_FULL,"[store_bq27531_access] read bq27531 reg 0x%x with value 0x%x !\n",reg_address,g_reg_value_bq27531);
            battery_xlog_printk(BAT_LOG_FULL,"[store_bq27531_access] Please use \"cat bq27531_access\" to get value\r\n");
        }        
    }    
    return size;
}
static DEVICE_ATTR(bq27531_access, 0664, show_bq27531_access, store_bq27531_access); //664
...
{    
    ...
    ret_device_file = device_create_file(&(dev->dev), &dev_attr_bq27531_access);
