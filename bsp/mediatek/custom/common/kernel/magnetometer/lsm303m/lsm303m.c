...
/*molg1 add soft groscope from FAE*/
#define LSM303D_GRO	
//#undef LSM303D_GRO
...
#define ECOMPASS_IOC_GET_GYFLAG			_IOR(MSENSOR, 0X57, int)
#endif
#ifdef LSM303D_VRV
...
#define ECOMPASS_IOC_GET_RVFLAG			_IOR(MSENSOR, 0X58, int)
#endif
...
/*----------------------------------------------------------------------------*/
//added by ST WU Yi 2013.9.5.
static int lsm303m_ReadSimuGyroData(char *buf, int bufsize)
{
	MSE_FUN(f);
	...
	read_lock(&lsm303mmid_data.datalock);
	sprintf(buf, "%d %d %d %d", lsm303mmid_data.simuGyrox, lsm303mmid_data.simuGyroy,
		lsm303mmid_data.simuGyroz, lsm303mmid_data.mag_status);
	read_unlock(&lsm303mmid_data.datalock);
	return 0;
}
...
#ifdef LSM303D_VRV
//added by ST WU Yi 2013.9.5.
static int lsm303m_ReadRotationVectorData(char *buf, int bufsize)
{
	MSE_FUN(f);
	...
	read_lock(&lsm303mmid_data.datalock);
	sprintf(buf, "%d %d %d %d", lsm303mmid_data.RVx, lsm303mmid_data.RVy,
		lsm303mmid_data.RVz, lsm303mmid_data.mag_status);
	read_unlock(&lsm303mmid_data.datalock);
	return 0;
}
#endif
...
#ifdef LSM303D_GRO
#ifdef LSM303D_VRV
                    //modified by ST WU Yi 2013.9.5.
					if(atomic_read(&o_flag) == 0 && atomic_read(&gy_flag) == 0 && atomic_read(&rv_flag) == 0)
#else
					if(atomic_read(&o_flag) == 0 && atomic_read(&gy_flag) == 0)
#endif
#else
...
#ifdef LSM303D_VRV
					//modified by ST WU Yi 2013.9.5.
					if(atomic_read(&m_flag) == 0 && atomic_read(&gy_flag) == 0 && atomic_read(&rv_flag) == 0)
#else
					if(atomic_read(&m_flag) == 0 && atomic_read(&gy_flag) == 0)
#endif
#else
...
/*----------------------------------------------------------------------------*/
//added by ST WU Yi 2013.9.5.
int lsm303m_simugyro_operate(void* self, uint32_t command, void* buff_in, int size_in,
		void* buff_out, int size_out, int* actualout)
{
	int err = 0;
	int value, sample_delay, status=0;
	...
	switch (command)
	{
		...
		case SENSOR_ENABLE:
			MSE_LOG("***simugyro SENSOR_ENABLE****");
			if((buff_in == NULL) || (size_in < sizeof(int)))
			{
				MSE_ERR("Enable sensor parameter error!\n");
				err = -EINVAL;
			...
			else
			{
				...
				{
					lsm303m_SetPowerMode( priv->client, 1);
					lsm303mmid_data.controldata[7] |= SENSOR_GYROSCOPE;
				...
				else
				{
					...
					if(atomic_read(&m_flag) == 0 && atomic_read(&o_flag) == 0 && atomic_read(&rv_flag) == 0)
					{
						atomic_set(&open_flag, 0);
					}
				}
		...
		case SENSOR_GET_DATA:
			//MSE_LOG("+++++++++++MSENSOR_GET_ORIENTATION_DATA");
			MSE_LOG("************simugyro SENSOR_GET_DATA***********\r\n");
			if((buff_out == NULL) || (size_out< sizeof(hwm_sensor_data)))
			{
				MSE_ERR("get sensor data parameter error!\n");
				err = -EINVAL;
			...
			else
			{
				...
				read_lock(&lsm303mmid_data.datalock);
				gysensor_data->values[0] = lsm303mmid_data.simuGyrox;
				gysensor_data->values[1] = lsm303mmid_data.simuGyroy;
				gysensor_data->values[2] = lsm303mmid_data.simuGyroz;
				status = lsm303mmid_data.mag_status;
...
/*----------------------------------------------------------------------------*/
//added by ST WU Yi 2013.9.5.
int lsm303m_rotation_vector_operate(void* self, uint32_t command, void* buff_in, int size_in,
		void* buff_out, int size_out, int* actualout)
{
	int err = 0;
	int value, sample_delay, status=0;
	...
	switch (command)
	{
		...
		case SENSOR_ENABLE:
			MSE_LOG("***rotation vector SENSOR_ENABLE****");
			if((buff_in == NULL) || (size_in < sizeof(int)))
			{
				MSE_ERR("Enable sensor parameter error!\n");
				err = -EINVAL;
			...
			else
			{
				...
				{
					lsm303m_SetPowerMode( priv->client, 1);
					lsm303mmid_data.controldata[7] |= SENSOR_ROTATION_VECTOR;
				...
				else
				{
					...
					if(atomic_read(&m_flag) == 0 && atomic_read(&o_flag) == 0 && atomic_read(&gy_flag) == 0)
					{
						atomic_set(&open_flag, 0);
					}
				}
		...
		case SENSOR_GET_DATA:
			...
			else
			{
				...
				read_lock(&lsm303mmid_data.datalock);
				rvsensor_data->values[0] = lsm303mmid_data.RVx;
				rvsensor_data->values[1] = lsm303mmid_data.RVy;
				rvsensor_data->values[2] = lsm303mmid_data.RVz;
				status = lsm303mmid_data.mag_status;
...
//	MSE_FUN(f);
	...
	switch (cmd)
	{
		...
		case MSENSOR_IOCTL_SET_SIMUGYRO:
			...
			write_lock(&lsm303mmid_data.datalock);
			 lsm303mmid_data.simuGyrox   = valuebuf[0];
			 lsm303mmid_data.simuGyroy = valuebuf[1];
			 lsm303mmid_data.simuGyroz  = valuebuf[2];
			 lsm303mmid_data.mag_status = valuebuf[3];
			write_unlock(&lsm303mmid_data.datalock);   
		...
		case MSENSOR_IOCTL_SET_ROTATION_VECTOR:
			...
			write_lock(&lsm303mmid_data.datalock);
			 lsm303mmid_data.RVx = valuebuf[0];
			 lsm303mmid_data.RVy = valuebuf[1];
			 lsm303mmid_data.RVz = valuebuf[2];
			 lsm303mmid_data.mag_status = valuebuf[3];
			write_unlock(&lsm303mmid_data.datalock);
...
//modified by ST WU Yi 2013.9.5.
        ...
        //added by ST WU Yi 2013.9.5.
			...
			lsm303m_ReadSimuGyroData(strbuf, LSM303M_BUFSIZE);
			if(copy_to_user(data, strbuf, strlen(strbuf)+1))
			{
				retval = -EFAULT;
				goto err_out;
        ...
        case MSENSOR_IOCTL_READ_ROTATION_VECTOR_DATA:
			...
			lsm303m_ReadRotationVectorData(strbuf, LSM303M_BUFSIZE);
			if(copy_to_user(data, strbuf, strlen(strbuf)+1))
			{
				retval = -EFAULT;
				goto err_out;
...
static int lsm303m_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#ifdef LSM303D_GRO
#ifdef LSM303D_VRV
		struct hwmsen_object sobj_m, sobj_o, sobj_gy, sobj_rv;
#else
		...
		sobj_gy.self = data;
		sobj_gy.polling = 1;
		sobj_gy.sensor_operate = lsm303m_simugyro_operate;
		...
		sobj_rv.self = data;
		sobj_rv.polling = 1;
		sobj_rv.sensor_operate = lsm303m_rotation_vector_operate;
