...
regdata = EPL_SENSING_16_TIME | EPL_PS_MODE | EPL_L_GAIN ;
	...
	else
	{
		 ...
		 if( data_cali>8000)
		{
		    ...
		    //ps_data_cali->far_away =data_cali +88;
    		    ps_data_cali->close =data_cali +1000;
		    ps_data_cali->far_away =data_cali +700;
                }
		else if((data_cali>0)&&(data_cali<1000))
		{
		    ps_data_cali->close =data_cali +800;
		    ps_data_cali->far_away =data_cali +550;
		}
		else
		{
...
//queue_delayed_work(epld->epl_wq, &polling_work,msecs_to_jiffies(350));
    ...
    if(enable_als && atomic_read(&epld->als_suspend)==0)
    {
    ...
    if(enable_ps && atomic_read(&epld->ps_suspend)==0)
    {
...
static int __init epl2182_init(void)
{
	...
	hwmsen_alsps_add(&epl2182_init_info);
	#if 0
