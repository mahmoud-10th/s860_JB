		...
		void *ramdisk, unsigned ramdisk_size)
{
    ...
    ptr = target_atag_commmandline(buf, cmdline);
...
//FIXME, Waiting LCM Driver owner to fix it
	...
	#ifdef LENOVO_LONG_POWER_RESET//lenovo jixu add
	...
	strlen += sprintf(commanline, "%s lcm_ver=%x", commanline,  mt_disp_get_lcd_version());
	//lenovo jixu add end
#ifdef DEVICE_TREE_SUPPORT
    if (boot_linux_fdt((void *)CFG_BOOTIMG_LOAD_ADDR, (unsigned *)CFG_BOOTARGS_ADDR,
		    (char *)commanline, board_machtype(),
		    (void *)CFG_RAMDISK_LOAD_ADDR, g_rimg_sz) == FALSE)
#endif
