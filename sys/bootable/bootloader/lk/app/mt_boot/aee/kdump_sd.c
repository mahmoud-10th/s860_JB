...
static int kdump_file_output(const struct kdump_params *kparams, uint32_t total_dump_size, struct mrdump_dev *mrdump_dev)
{
    if (mrdump_dev == NULL) {
        return -1;
    }
	...
	bool ok = true;
        void *bufp = kdump_core_header_init(kparams, (uint32_t)kparams->phys_offset, total_dump_size);
	if (bufp != NULL) {
            mtk_wdt_restart();
...
int kdump_emmc_output(const struct kdump_params *kparams, uint32_t total_dump_size)
{
    kdump_file_output(kparams, total_dump_size, mrdump_dev_emmc());
    return 0;
}

int kdump_sdcard_output(const struct kdump_params *kparams, uint32_t total_dump_size)
{
